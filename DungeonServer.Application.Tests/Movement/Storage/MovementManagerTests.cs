...
namespace DungeonServer.Application.Tests.Movement.Storage;
...
public class MovementManagerTests
{
    ...
    public async Task SetMovementInput_Applies_WhenWithinSpeedLimit()
    {
        ...
        var snapshot = await playerStore.CreatePlayerAsync(Location.Origin, CancellationToken.None);

        var requested = new Location(1f, 0f);  

        var resp = await manager.SetMovementInput(new MovementInputRequest(snapshot.PlayerId, requested), CancellationToken.None);
